...
; sub _process
   ...
   ; do { local $Class::props::force = 1
        ; $z->mt = $s           # init top main zone mt is a class property
        }
   ; $z->content_process
...
; sub _parse
   ...
   ; Text::MagicTemplate::Zone->new( _s      => 0
                                   , _e      => $#temp
                                   , _t      => \@temp
                                   , is_main => 1
                                   )
...
If you want to install Text::MagicTemplate plus all related extensions (the prerequisites to use also L<HTML::MagicTemplate|HTML::MagicTemplate>), all in one easy step:
    ...
    perl -MCPAN -e 'install Bundle::MagicTemplate'
...
=item Distribution structure
    ...
    Bundle::MagicTemplate        a bundle to install everything in one step
...
If you need to use Text::MagicTemplate with C<CGI::Application> (that requires the run modes method to collect the whole output) you may use L<CGI::Application::CodeRM|CGI::Application::CodeRM> that allows you to use C<print()> or C<nprint()> method instead of C<output()> method.
...
B<Note>: to mantain backward compatibility, you can use the old constructor arrays identifiers with the prepended '-' and/or pass a reference to a hash:
...
If you want to use the HTML handlers too, you could use HTML::MagicTemplate. See L<HTML::MagicTemplate> for details.
