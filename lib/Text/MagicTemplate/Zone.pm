...
; sub value_process
   ...
   ; my $ch = $mt->{value_handlers} || return
   ; HANDLER:
     foreach my $h ( @$ch )
      { return OK if &$h(@_)
      }
...
This method loads and process a template. It is useful specially if you want to conditionally load and process a template from inside your code. (see L<Text::MagicTemplate/"Conditionally include and process a template file">)
...
The following are the properties that Text::MagicTemplate uses to do its job: they all are left value properties.
